...
scp -i "%SSH_KEY%" compose.yml mailserver.env roundcube.php common.sh build.sh accounts.sh add-account.sh setup-tls.sh nginx-http.conf nginx-https.conf dkim.sh dkim-signing.conf fail2ban-jail.cf ignore-roundcube.sh "%TARGET%:%REMOTE_DIR%/"
if errorlevel 1 goto error
