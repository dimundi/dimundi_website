...
set "REMOTE_DIR=/home/docker/dimundi/whatthefrog/deploy"
...
scp -i "%SSH_KEY%" compose.yml mailserver.env roundcube.php common.sh build.sh accounts.sh setup-tls.sh nginx-http.conf nginx-https.conf dkim.sh dkim-signing.conf postfix-virtual.cf fail2ban-jail.cf ignore-roundcube.sh "%TARGET%:%REMOTE_DIR%/"
if errorlevel 1 goto error
echo Pliki wyslane. Kontenery nie zostaly uruchomione.
echo Instrukcja: dimundiWwwDocs/instalacja/whatthefrog/email.md.
exit /b 0
