...
apply_config() {
    local source=$1 target="$PROXY_DIR/mail-conf/poczta.conf" backup
    backup=$(mktemp)
    local had_config=0
...
if [[ ! -f "$PROXY_DIR/mail-conf/https.enabled" ]]; then
    confirm 'Wlaczyc obsluge HTTP-01 dla poczta.dimundi.com i webmail.dimundi.com?' || exit 0
...
args=(certonly --webroot -w /var/www/certbot --cert-name poczta.dimundi.com
    -d poczta.dimundi.com -d webmail.dimundi.com --email "$email" --agree-tos --non-interactive)
...
touch "$PROXY_DIR/mail-conf/https.enabled"
