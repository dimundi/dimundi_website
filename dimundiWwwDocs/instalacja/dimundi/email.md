...
Instrukcja postawienia środowiska od zera: [email-instalacja.md](../wspolne/email-instalacja.md). Wdrożenie strony i proxy: [deploy.md](../wspolne/deploy.md).
...
Migracja WhatTheFrog: [osobne ustalenia](../whatthefrog/email.md).
...
Dodawanie kolejnej skrzynki: na VPS jako `docker` uruchom `cd ~/dimundi/poczta/deploy`, następnie `bash add-account.sh`. Skrypt pyta o nazwę/adres `.com`, opcjonalny alias `.pl` i potwierdzenie `t/n`; hasło podaje się w ukrytym monicie DMS. Sprawdza kolizje z istniejącymi kontami i aliasami, bez nadpisywania haseł. Przy catch-all dodaje dla nowego konta jawne mapowanie na siebie, aby poczta trafiała do jego skrzynki. Wymaga działającego kontenera. Plik wysyła istniejący `poczta/dimundi/deploy/install.bat`. `accounts.sh` pozostaje do pierwszej instalacji Marcina, Beaty i aliasów technicznych.

- Repo: `poczta/dimundi/deploy/`, projekt Compose `dimundi-mail`. Instalatory kopiują przez SSH/SCP; na VPS nie używamy Gita.
